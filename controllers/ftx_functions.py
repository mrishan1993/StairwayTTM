...
class FtxClient:
    ...
    def __init__(self, api_key=None, api_secret=None, subaccount_name=None) -> None:
        ...
        self._api_key = "2hwDFYhkje7CrFI_Gj3KdWNZBEIVXHOK02mbw_qP" # api key
        self._api_secret = "SZo9nXzbn-i1V_xkb8oPP-yYF5sr0VWuEgSOqA0A" # api secret
    ...
    def place_conditional_order(
        self, market: str, side: str, size: float, type: str = 'stop', trigger_price: float = None,
...
def start_process():
    ...
    if (markets[0]["side"] == "buy" and markets[0]["status"] == "closed" and type(markets[0]["avgFillPrice"]) == float and markets[0]["size"] > 0.01):
        ...
        size = markets[0]["size"]
        print(current_price)
        if (current_price >= 1.014*buying_price or current_price < 0.993 * buying_price):
        ...
        for i in markets: 
            ...
            if (len(current_positions)) > 0:
                ...
                if (position and len(position) > 0 and position["Position"] == "Buy"):
                    print ("buying price ", position["BuyingPrice"])
                    size = balance / ( position["BuyingPrice"])
                    print ("size ", size)
